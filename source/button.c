...
void Button_Init(button_t* button, button_data_t data, C2D_SpriteSheet sheet) {
    ...
    button->pressed_color = ButtonData_GetPressedColor(data.color);

    C2D_SpriteFromSheet(&button->sprite, sheet, button->data.sprite_idx);
    C2D_SpriteSetCenter(&button->sprite, 0.5f, 0.5f);
    C2D_SpriteSetPos(&button->sprite, button->data.pos.x + (button->data.size.x / 2),
                     button->data.pos.y + (button->data.size.y / 2));
}
...
void Button_Draw(button_t* button) {
    ...
    if (Button_IsDown(button)) {
        ...
        C2D_DrawRectSolid(button->data.pos.x, button->data.pos.y, 0, button->data.size.x, button->data.size.y,
                          ColorF_ToU32(button->data.color));
...
bool Button_IsReleased(button_t* button) {
    return (button->pressable && !Input_IsTouchScreenDown() && !Button_IsDown(button) && button->held == true);
}
