...
CFLAGS	+=	$(INCLUDE) -I$(PORTLIBS)/include/opus -D__3DS__ -DHAVE_SINF
...
LIBS	:=  -lopusfile -lopus -logg   -lcitro2d -lcitro3d -lctru -lm 
...
#---------------------------------------------------------------------------------
	@echo $(notdir $<)
	@tex3ds -i $< -H common/$*.h -d "$(DEPSDIR)/$*.d" -o "$(GFXBUILD)/$*.t3x"
